...
def combine_scores(points, ref_point=None):
   ...
   try:
       ...
       for point in points_array:
           # Check if point is valid (no NaN or inf)
           if np.any(np.isnan(point)) or np.any(np.isinf(point)):
               continue
          
           # Check if point dominates reference point (for minimization, lower is better)
           if len(point) >= 2:
       ...
       if len(filtered_points) == 0:
           ...
           # Convert to numpy array for hypervolume calculation
...
def print_solution_summary(x, udp_instance=None):
   """
   Print a summary of a solution
  
   Parameters:
   -----------
   x : list or np.ndarray
       Decision vector (20 elements)
   udp_instance : constellation_udp, optional
       UDP instance for fitness evaluation
   """
   try:
       if len(x) != 20:
       ...
       # Walker constellation information
       ...
       # Evaluate fitness if UDP instance is provided
       if udp_instance is not None:
           try:
               from constellation_udp import constellation_udp
               if udp_instance is None:
                   udp_instance = constellation_udp()
...
def check_dependencies(verbose=True):
   ...
   Returns:
   --------
   tuple : (all_available, critical_available, dependencies_dict)
   """
   dependencies = {
...
def validate_solution(x, udp_instance=None):
   ...
   try:
       ...
       # Note: We don't know the range without the rovers database
       # So just check they're non-negative
